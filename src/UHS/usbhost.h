...
void MAX3421e< SPI_SS, INTR >::busprobe(bool regardless) {
        ...
        if (regRd(rHIRQ) & bmCONDETIRQ) {
            // device plug/unplug was detected by the chip
            // clear CONDETIRQ
...
uint8_t MAX3421e< SPI_SS, INTR >::Task(void) {
        ...
        /* tmk patch for the chip missing plug-in events
        // - skip checking the INT pin because it seems sometimes to miss connect events
        // - just call busprobe() instead
        uint8_t rcode = 0;
        uint8_t pinvalue;
